...
function _logical_axis_label(observable::Symbol)
    observable === :logical_x && return "Logical-X failure rate"
    observable === :logical_z && return "Logical-Z failure rate"
...
function _comparison_panel!(position, scan::ChannelFailureScan, fit::ThresholdFit; title::AbstractString)
    ...
    axis = CairoMakie.Axis(
        ...
        ylabel=_logical_axis_label(scan.logical_observable),
    )
    ...
    if fit.status === :success
        ...
        inset = CairoMakie.Axis(
            ...
            xlabel="(p − p₍c₎) d¹ᐟᵛ", ylabel="failure", titlesize=11,
