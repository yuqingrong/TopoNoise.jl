...
source-check blocks for `:plaquette`. Logical preparation and correction
blocks receive faults only under `:gate_layer`. With `:post_encoding`, the
entire preparation is perfect, then one IID X/Z layer acts on every data
qubit before the perfect final syndrome round. The declared product input
is ideal: native As starts in all-plus and Bp in all-zero. Representative
H gates in their stored schedules remain eligible for gate-layer noise.
"""
struct CircuitPauliNoise
    ...
    function CircuitPauliNoise(p_x::Float64, p_z::Float64, clock::Symbol)
        ...
        clock in (:gate_layer, :plaquette, :post_encoding) ||
            throw(ArgumentError("clock must be :gate_layer, :plaquette, or :post_encoding"))
...
struct CircuitFaultStep
    ...
    function CircuitFaultStep(
        ...
        block_kind in (:logical_sector, :plaquette, :post_encoding) ||
...
struct CircuitFaultRecord
    ...
    function CircuitFaultRecord(
        ...
        clock in (:gate_layer, :plaquette, :post_encoding) ||
            throw(ArgumentError("fault-record clock must be :gate_layer, :plaquette, or :post_encoding"))
...
function _eligible_fault_steps(encoder::PlaquetteEncoder, clock::Symbol)
    if clock === :post_encoding
        return [(1, :post_encoding, 0, collect(1:data_qubit_count(encoder.code)))]
    end
    if clock === :gate_layer
        return [
...
function _validate_fault_record(
    ...
    code = encoder.code
    record.clock in (:gate_layer, :plaquette, :post_encoding) ||
        throw(ArgumentError("fault-record clock must be :gate_layer, :plaquette, or :post_encoding"))
...
function propagate_pauli_frame(
    ...
    if faults.clock === :post_encoding
        # Perfect preparation leaves the relative error frame empty. No
        # encoder gates follow this fault, so it must not be propagated.
        _inject_frame_step!(x, z, only(faults.steps))
    elseif faults.clock === :gate_layer
...
function _apply_yao_encoder_and_faults!(
    ...
    # The register allocation is all-zero. Prepare the declared ideal data
    # input before replaying the noisy schedule, leaving the readout ancilla zero.
    for operation in _input_preparation_operations(encoder)
        apply!(register, _yao_operation(qubit_count, operation))
    end
    if faults.clock === :post_encoding
        for layer in gate_layers(encoder), operation in layer.operations
            apply!(register, _yao_operation(qubit_count, operation))
        end
        _apply_yao_fault_step!(register, qubit_count, only(faults.steps))
    elseif faults.clock === :gate_layer
        for (layer, step) in zip(gate_layers(encoder), faults.steps)
            for operation in layer.operations
                apply!(register, _yao_operation(qubit_count, operation))
