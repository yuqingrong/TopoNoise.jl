...
function _crossing_candidates(lower, upper)
    ...
    for index in eachindex(lower.rates)
        ...
        if delta == 0.0 && lower.values[index] != 0.0
...
"""Choose the unique tightest cluster of one finite-p crossing per distance pair."""
function _consistent_crossing_selection(candidate_sets)
    best = nothing
    best_spread = Inf
    tied = false
    selected = PairCrossing[]

    function visit(index::Int)
        if index > length(candidate_sets)
            spread = maximum(crossing.p for crossing in selected) -
                minimum(crossing.p for crossing in selected)
            if best === nothing
                best = copy(selected)
                best_spread = spread
                tied = false
            else
                tolerance = 64eps(max(1.0, abs(best_spread), abs(spread)))
                if spread < best_spread - tolerance
                    best = copy(selected)
                    best_spread = spread
                    tied = false
                elseif abs(spread - best_spread) <= tolerance
                    tied = true
                end
            end
            return
        end
        for crossing in candidate_sets[index]
            push!(selected, crossing)
            visit(index + 1)
            pop!(selected)
        end
    end
    ...
    visit(1)
    return tied ? nothing : best
end

"""Return one mutually consistent finite-p crossing for every adjacent pair."""
function _adjacent_crossings(scan::ChannelFailureScan)
    curves = _scan_curves(scan)
    candidate_sets = Vector{Vector{PairCrossing}}()
    for index in 1:(length(curves) - 1)
        ...
        push!(candidate_sets, candidates)
    end
    crossings = _consistent_crossing_selection(candidate_sets)
    crossings === nothing && return PairCrossing[],
        "ambiguous finite-p crossing choices between adjacent distances"
...
function _with_selected_channel_count(
    ...
    if observable === :logical_z
        return LogicalFailurePoint(
            ...
            point.state_failures, point.state_failure_rate,
            point.state_standard_error)
    elseif observable === :state_failure
        return LogicalFailurePoint(
            point.distance, point.boundary_orientation, point.construction,
            point.logical_state, point.clock, point.p_x, point.p_z,
            point.shots, point.seed, point.logical_x_failures,
